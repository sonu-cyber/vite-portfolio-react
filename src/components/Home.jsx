...
(console.log('showLink is NOW:', link.showLink, 'link id is now:',link.id, 'id is:', id,'link.url:',link.url, 'link is now:', link));
return link;
 ...
 /* Deleting a link */
  ...
  /* Return values */
    ...
    <div>
      {links.length >0 ? <p><DelLink links={links}  onDelete={deleteLink} onToggle = {toggleShowLink} /></p>: 'No links to show!'}
      {<AddLink links={links} onAdd={linkAdd}/>} 
      </div>    
     
      <div className={styles.goback} ><Link to="/">Go back</Link></div>
    </div>
